package com.pisane.pisane.controler.background_worker.common

const val base_url = "https://piotrkroldev.online"

const val passwordhash_php = "passwordhash.php"
const val passwordhash_url = "${base_url}/${passwordhash_php}"

const val register_php = "register.php"
const val register_url = "${base_url}/${register_php}"

const val highscores_php = "highscores.php"
const val highscores_url = "${base_url}/${highscores_php}"

const val new_highscore_php = "newhighscore.php"
const val new_highscore_url = "${base_url}/${new_highscore_php}"

const val new_cards_set_component_php = "newcardssetcomponent.php"
const val new_cards_set_component_url = "${base_url}/${new_cards_set_component_php}"

const val new_cards_set_php = "newcardsset.php"
const val new_cards_set_url = "${base_url}/${new_cards_set_php}"

const val card_sets_php = "cardsets.php"
const val card_sets_url = "${base_url}/${card_sets_php}"

const val is_set_played_php = "issetplayed.php"
const val is_set_played_url = "${base_url}/${is_set_played_php}"

const val new_played_set_php = "newplayedset.php"
const val new_played_set_url = "${base_url}/${new_played_set_php}"
